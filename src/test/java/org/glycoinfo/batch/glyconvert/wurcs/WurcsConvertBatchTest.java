 ...
 * @author aoki
 *
 *         Test cases for the wurcs converter. It will search for a number of
 *         glycoct that are not converted yet, store the number, execute the
 *         batch process. If the number is less after the batch, it is
 *         successful.
 *
...
//	@Test
	...
	public void testJob() throws Exception {
		...
		try {
			...
			// catch any Convert exceptions and just display them, see known issue below.
		...
		// https://github.com/glytoucan/glytoucan.github.io/issues/26
...
//		Assert.assertNotEquals(count, countAfter);
	...
	/*
	 * PREFIX glycan: <http://purl.jp/bio/12/glyco/glycan#> SELECT DISTINCT
	 * ?SaccharideURI ?id ?SeqWurcs ?Sequence FROM
	 * <http://rdf.glycoinfo.org/glycome-db> WHERE { ?SaccharideURI
	 * glycan:has_glycosequence ?GlycanSequenceURI .
	 * BIND(STRAFTER(str(?SaccharideURI), "http://rdf.glycome-db.org/glycan/")
	 * AS ?id ) ?GlycanSequenceURI glycan:has_sequence ?Sequence .
	 * BIND(CONCAT(STRBEFORE(str(?GlycanSequenceURI), "/ct"), "/wurcs/2.0") AS
	 * ?SeqWurcs) ?GlycanSequenceURI glycan:in_carbohydrate_format
	 * glycan:carbohydrate_format_glycoct . }
	 * 
	 * PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX glycan:
	 * <http://purl.jp/bio/12/glyco/glycan#> PREFIX glytoucan:
	 * <http://www.glytoucan.org/glyco/owl/glytoucan#> INSERT INTO GRAPH
	 * <http://rdf.glycoinfo.org/glycome-db/sequence/wurcs> {
	 * <testSaccharideURI> glycan:has_glycosequence
	 * <http://www.glycoinfo.org/rdf/glycan/5963/wurcs/2.0> .
	 * <http://rdf.glycome-db.org/glycan/5963/wurcs/2.0> glycan:has_sequence
	 * "testConvertedSequence!"^^xsd:string .
	 * <http://rdf.glycome-db.org/glycan/5963/wurcs/2.0>
	 * glycan:in_carbohydrate_format glycan:carbohydrate_format_wurcs .
	 * <http://rdf.glycome-db.org/glycan/5963/wurcs/2.0>
	 * glytoucan:is_glycosequence_of <testSaccharideURI> . }
	 */
}