 ...
 *
 */
public class TtlItemWriter<T extends SparqlEntity> extends
		AbstractItemStreamItemWriter<T> implements
		ResourceAwareItemWriterItemStream<T>, InitializingBean {

	protected static final Log logger = LogFactory.getLog(TtlItemWriter.class);

	public TtlItemWriter() {
		this.setExecutionContextName(ClassUtils
				.getShortName(TtlItemWriter.class));
	}
...
//					insertSparql.setPrefix(null);
//					filewriter.write(insertSparql.getSparql());
//				}
//			}
//			filewriter.close();
//		} catch (IOException e) {
...
//		Assert.notNull(lineAggregator, "A LineAggregator must be provided.");
	 ...
	 *            the {@link LineAggregator} to set
	 */
...
//	}
	 ...
	 *             WriterNotOpenException if the writer has not been
	 *             initialized.
	 */
	@Override
	public void write(List<? extends T> items) throws Exception {
		...
		int lineCount = 0;
		for (T item : items) {
			InsertSparql insert = getInsertSparql();
			insert.setSparqlEntity(item);
			lines.append(insert.getInsert());
